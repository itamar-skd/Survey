...
module.exports = {
    ...
    callback: async (message, args, client) => {
        ...
        do {
            ...
            if (epic === 'cancel') return message.reply('Cancelled.')
        } while (!channel && entry !== 'cancel')
        await Guild.findOne({
            ...
            if (!channelNew) {
                ...
                const currentChannel = channelNew.announcementsChannelID ? message.guild.channels.cache.get(channelNew.announcementsChannelID) : ''
                if (channel.id === currentChannel.id) return message.reply('That\'s already the survey announcements channel!')
                Guild.updateOne({
                    guildID: message.guild.id
                }, {
                    guildID: message.guild.id,

        ...
        if (emoji === 'x_') return msg.channel.send('Cancelled.')
            ...
            for (const question of guild.survey) {
                ...
                if (type === 'long') {
                    ...
                    condition = `['yes', 'no'].every(element => element !== reply.toLowerCase())`
                    reason = 'Please only enter Yes / No answers.'
                }
                const characters = type === 'long' ? `(${Math.floor(1500 / guild.survey.length)} characters)` : type === 'short' ? `(50 characters)`: type === 'number' ? '(Number only.)' : '(Please type Yes / No **only**.)'
